	...
	function closeAllExcept(id){
		...
		for(i; i < 6; i++ ){
			if(ids[i] != '#'+id){
				...
				$(".picker").addClass("board").removeAttr("id");
			} 
		}
	}
...
// color picker 
  ...
  function colorPicked(hex){
  	...
  	if(getSide() == 'front'){
  		...
  		setFBoardColor(color);
  	}
  	else{
  		$('#snowboarding_editor').css("background",color); 	
  		setBBoardColor(color);
  	}
  	return;
  } 
	...
	*************************************************************/
						...
						case 4:
							...
							if(num != -1){
								if(getCurrentPointerPositionY() != getYP3(num) && getCurrentPointerPositionY() != getYP4(num) ){
									diff = parseFloat(getCurrentPointerPositionY() - getYP2(num))*EXTEND_RATE;
								...
								if((getCurrentPointerPositionX() != getXP3(num) && getCurrentPointerPositionX() != getXP4(num))							
									...
									diff = parseFloat(getCurrentPointerPositionX() - getXP3(num))*EXTEND_RATE;
									console.log("new width " + diff);
									setImageWidth(num, diff);						
								}
								draw(false,false);
	...
	function setFrontData(){
		frontData = {
			...
			"fonts":getFontsDATA(),
			"backgroundColor":getFBoardColor()
		}
		console.log("frontData" + frontData["side"] );
	}
	...
	function setBackData(){
		...
		console.log("backData" + backData["side"] );
	}

	function restoreFrontData(){
		clearAll();
		if(frontData != null){
			//console.log(frontData["images"][0]["xp1"]);
			var images = frontData["images"];
			var fontd = frontData["fonts"];
			...
			for(i; i < fontd.length; i++){
				fontStartX[i] = fontd[i]['fxp1'] + fontd[i]['width']/2;
				fontStartY[i] = fontd[i]['fyp1'] + fontd[i]['height']/2;
				fontWidth[i] = fontd[i]['width'];
				fonts[i] = fontd[i]['fonts'];
				fontFamilies[i] = fontd[i]['fontFamily'];
				fontSizes[i] = fontd[i]['size'];
				strs[i] = fontd[i]['text'];
				fontRotation[i] = fontd[i]['angle'];
				fontCounts = fontd[i]['fontCounts'];
				fontColors[i] = fontd[i]['fontColor'];
				console.log(
					"fontStartX " + fontStartX[i] + " fontStartY " + fontStartY[i] + " fontWidth " + fontWidth[i] + " fonts "+ fonts[i] +
					" fontFamilies " + fontFamilies[i] + " fontSizes "+ fontSizes[i] + " fontRotation " + fontRotation[i] + " strs " + strs[i] +
	...
	function restoreBackData(){
		clearAll();
		if(backData != null){
			//console.log(backData["side"]);
			var images = backData["images"];
			var fontd = backData["fonts"];
	...
	function getFontsDATA(){
		...
		for(ind; ind < fontCounts; ind++){
			font[ind] = {
				"text":getString(ind),"fxp1":getFXP1(ind),	"fxp2":getFXP2(ind),	"fxp3":getFXP3(ind),	"fxp4":getFXP4(ind),"fyp1":getFYP1(ind),	"fyp2":getFYP2(ind),	"fyp3":getFYP3(ind),	"fyp4":getFYP4(ind),	
				...
				"afyp4":getAngledFYP4(ind),"angle":getAngle(ind,true),"width":getFontWidth(ind),"height":getFontHeight(ind), "scale":getScale(),"fontColor":getFontColor(ind), "fonts":getFontString(ind),
				"size":getFontSize(ind), "fontFamily":getFontFamily(ind), "fontCounts":fontCounts
			}
			d[ind] = font[ind]; 
		}
		return d;	
